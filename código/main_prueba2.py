...
OLLAMA_URL="http://localhost:11434/api/chat" #generate / chat
...
#post name
def postName():
    if getGente().get("bunnydos") is None:
        requests.post(SERVER_URL+"alias/bunnydos")
...
async def buzon(request: Request, mensaje: Mensaje):
    ...
    #recursos
    mis_recursos = getRecursos()

    #negociacion
    prompt_sistema = f""" Eres un jugador experto de Catan. Tus recursos actuales son: {mis_recursos}

    Reglas:
    - Analiza si la oferta te conviene.
    ...
    r = requests.post(
        ...
        json={
            "model": DEFAULT_MODEL,
            "messages":[{#'role': 'system', 'content': 'Eres un jugador de Catan que tieen que negociar con otro y ver si los recursos que te piden los tienes tu'},
            ...
            "format": {
                ...
                "required": ["recursos", "cantidad", "accion"]
            },
    ...
    #recursos = ListaRecursos.model_validate_json(r.json.get("response", ""))
    ...
    try:
        ...
        recursos = ListaRecursos.model_validate_json(contenido)
        print('JSON válido:', recursos)
        
    except Exception as e:
        print("Error parseando JSON:", e)
    
    send_data = {
        "msg": "hola, somos bunnydos :)"
    } 
...
if __name__ == "__main__":
    ...
    thread.start()
    uvicorn.run(app, host=getGente().get("bunnydos"), port=7720)
